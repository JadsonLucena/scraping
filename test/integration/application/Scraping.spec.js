...
import Scraping from '../../../src/application/Scraping'

const url = new URL('https://example.com/home')
const USER_AGENT = 'Robots'
const PAGE_SIZE = {
  width: 1280,
  height: 720
}
...
test('Given that you want to get all the information from a website', async () => {
  await expect(Scraping(browser, url)).resolves.toEqual(expect.objectContaining({
  ...
  return expect(Scraping(browser, url, {
    fields: [
      'title',
      'description',
      'favicons',
...
test('Given that you want to get the favicons of a website', async () => {
  return expect(Scraping(browser, url, {
    fields: ['favicons'],
    userAgent: USER_AGENT
  })).resolves.toMatchObject({
...
test('Given that you want to get the all headers of a website', async () => {
  return expect(Scraping(browser, url, {
    fields: ['h*'],
    userAgent: USER_AGENT
  })).resolves.toMatchObject({
...
test('Given that you want to get the some headers of a website', async () => {
  return expect(Scraping(browser, url, {
    fields: [
      'h2',
      'h3-4',
...
test('Given that you want to get the screenshot of a website', async () => {
  return expect(Scraping(browser, url, {
    fields: ['screenshot'],
    userAgent: USER_AGENT
  })).resolves.toMatchObject({
...
test('Given that you want to get the information from a website but robots are not allowed', async () => {
  await expect(Scraping(browser, new URL(`${url.origin}/login`), {
    userAgent: USER_AGENT
  ...
  return expect(Scraping(browser, new URL(`${url.origin}/login`), {
    userAgent: USER_AGENT,
    ignoreDisallowedRobots: true
  })).resolves.toEqual(expect.objectContaining({
    description: expect.any(String),
    favicons: expect.any(Array),
    headers: expect.any(Object),
