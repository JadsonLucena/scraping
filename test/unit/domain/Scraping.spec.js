...
test('Given you want to call function with invalid arguments', async () => {
  for (const input of INVALID_INPUT_TYPES.concat([], undefined)) {
    await expect(Scraping(input)).rejects.toThrowError(new TypeError('Invalid page'))
  }
  await expect(Scraping({})).rejects.toThrowError()
  for (const input of INVALID_INPUT_TYPES) {
    await expect(Scraping(page, {
      fields: input
    })).rejects.toThrowError(new TypeError('Invalid fields'))
  }
  for (const input of INVALID_INPUT_TYPES.concat([])) {
    await expect(Scraping(page, {
      userAgent: input
